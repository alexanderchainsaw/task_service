...
async def test_fetch_new_tasks_for_publishing(task_repository, test_session):
    ...
    task4 = create_task(task_name=TaskName.COMPRESS_IMAGE, priority=TaskPriority.HIGH, status=TaskStatus.PENDING)
    ...
    assert len(direct_our_tasks) == 3, f"Direct query found {len(direct_our_tasks)} tasks, expected 3. Task IDs: {[t.id for t in direct_our_tasks]}"
    ...
    if our_tasks:
        assert all(t.status == TaskStatus.NEW for t in our_tasks), f"Expected all NEW tasks, got: {[t.status for t in our_tasks]}"
    ...
    if len(our_tasks) >= 2:
        ...
        high_idx = next((i for i, t in enumerate(our_tasks) if t.priority == TaskPriority.HIGH), None)
