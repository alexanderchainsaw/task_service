...
async def test_process_message_task_not_found():
    ...
    with patch("app.workers.consumer.TaskRepository", return_value=mock_repository):
...
async def test_execute_task_unregistered_task():
    ...
    with patch(
        "app.workers.consumer.get_task_function",
        side_effect=ValueError("Task SEND_EMAIL is not registered in the registry"),
    ):
